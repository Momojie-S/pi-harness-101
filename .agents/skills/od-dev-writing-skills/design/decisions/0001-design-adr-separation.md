...
- 单文件「decisions 日志」(如某子项目 `decisions.md` 的 D-NN 紧凑条目)是 ADR-lite,但丢了 ADR 最值钱的 **Context**(当初面对的问题)和 **Consequences**(代价 + follow-up);且单文件难承载 rich 条目、supersede 链别扭。
...
- **follow-up**:① 仍用单文件 decisions 日志的子项目(若有),迁到 `decisions/` ADR-per-file + INDEX;② 其他已有 skill 的 design.md 逐个迁。
