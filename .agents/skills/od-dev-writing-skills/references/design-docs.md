...
> 本文件是 `od-dev-writing-skills` 硬规范 #1 的细则。创建 / 改 skill(或子项目)时按此组织设计文档。
...
> Context 写给**没参与当时决策**的维护者:只依赖读者默认知道的,默认之外的具体(别项目的玩法 / NPC / 私有编号)匿名 / 泛化 / 解释 —— 详见 §3.6。
...
> **别在 ADR 里拿本 skill(通用写作规范)当理由引用**:4 硬规范 / 两类框架 / writing-craft 是对所有 skill 默认成立的**基线**,非某 skill 的专属决策。「我们遵循规范 X(见 ADR-XXXX)」= 重申基线 = boilerplate(每个 skill 都会这么写)→ 不 ADR-worthy。ADR 只记**该 skill 特有**的决策,用自己的话讲清楚;只有**故意偏离**某通用规范时,才值得 ADR 引用它说明为何偏离。
...
- **skill vs 子项目**:skill 的 design 通常轻(方法论 + 几条 ADR);子项目(如某大型玩法)design 重(多关注点文档 + 多 ADR)。**结构同,深度不同**。
...
### 3.6 ADR 写得让未来读者看得懂(默认知识范围 + 圈外具体怎么处理)

ADR 给**没参与当时决策的维护者**看。可读性底线:**只依赖读者默认能知道的东西;默认之外的具体,匿名 / 泛化 / 当场解释。**

**默认知识范围,由 ADR 所在位置定(累加):**

| ADR 位置 | 读者默认知道的 |
|---|---|
| 公共 skill(`od-dev-*`)| 本仓 + 一条龙公共包(`one_dragon`)+ 行业通用词 |
| 项目内 skill / 子项目(项目专属开发 skill、某玩法 design)| 上面 + **本项目代码 / 文档 / 约定 / 游戏事实** |

同一样东西,在项目 ADR 里是"已知",在公共 skill ADR 里就是"泄漏" —— 例:具体玩法名、某 NPC、项目私有 bug 编号,写在公共 skill ADR 里,别的项目(如 ZZZ)读者读不懂。

**写 / 改时,对每个具体且关键的东西(删了就读不懂的那种)判它在不在默认知识里:**

- **在** → 直接用;不显然的首次带一句解释(如 `screen_info` = 每张画面的区域 / 模板定义,框架靠它识别画面)。
- **不在** → 三招:
  1. **匿名**(首选):保留具体感,不点名。"某自走棋式玩法"代具体玩法名、"一个 NPC"代具体 NPC。
  2. **泛化**:只留通用结论。"有些游戏用非文字符号标可交互对象"代某 NPC 的 `>名字<` 标记。
  3. **当场解释**:具体有说明价值就留,框成"某项目的做法"并讲清。

> 注:这条管的是**理解 ADR 必须知道、却不在默认知识里的事实**;不管"运行时读写的位置"(告诉 agent 读项目 `.debug/` 日志、写 `docs/game/` 是合法运行时操作,每个项目都有 —— 见 §5.1 范围)。

**读者自检**:写完问 —— 一个合格同行,带着上面"默认知道的",没参与过当时的事,能不跳出默认知识就读懂吗?他要去翻默认知识之外的东西(别项目代码、私有 bug 表、某次日志)→ 这条漏了。

这是规范 3「自包含」的细化:自包含是相对一个**位置相关的默认知识范围**说的(规范 3「项目内 skill 可引项目路径」即本条的项目情况)。

**另一半(全在默认知识内仍可能看不懂)**:写成只给自己看的简略笔记 —— 只甩标签不讲当时情况、长串嵌套修饰、造短词代概念。修法:Context 先讲"当时发生了啥"再讲"决定啥";决策一句大白话放开头;别造短词,说大白话(这条主要压项目内密集日志,如单文件 D-NN)。
