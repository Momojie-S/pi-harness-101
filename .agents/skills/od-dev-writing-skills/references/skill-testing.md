...
> 本文件是 `od-dev-writing-skills` 的测试方法。**写 / 改 skill 后按此验证**(GREEN 不可省,见 SKILL.md「两类 skill」)。
...
2. 它推进;遇到不清楚的会**问你**。
3. 你**只答所问**(非引导式主持,可用性测试手法):它问"X 放哪"你答路径;它问"选 A 还是 B"你回"你自己定,按 skill" —— **绝不**替它选 / 替它做。
