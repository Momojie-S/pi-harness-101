...
这里收录**反复踩过的坑**，提炼成硬纪律。每条都是“又犯了”才值得记——零星失误不记。

### 1. 交互反馈：用户触发的每个操作都要有可见反馈

**陷阱**：功能逻辑做完了（后端执行成功），但用户端没有任何提示——命令执行无确认、操作完成无反馈。用户不知道“到底发生没有”。这是最高频的交互缺陷：开发者只关注“后端做了什么”，忽略“用户看到了什么”。

**纪律**：用户触发的每个操作（命令、按钮、快捷键）都必须有**可见反馈**，成功和失败都要。后端返回结果时，前端必须有对应的 UI 响应（不能默默 dispatch 一个状态更新就完事）。

**具体到 web-console**：
- 成功/信息反馈用 `system-notice` 消息（绿色，进消息流，对称于 `system-error`）。
- 失败反馈用 `system-error`（或 `globalError`）。
- 反馈进消息流（按时间顺序），不要用浮动 toast——消息流有上下文，能看出“什么时候发生了什么”。
- 新加 builtin 命令（`/xxx`）时，**必须同时实现它的成功反馈**，不能只发 WS 消息不管结果。

**自查**：加完一个功能后问自己——“用户操作后，能不能从界面上确认它成功了？”看不到就补反馈。
...
├── docs/                     # 学习笔记与调研
│   └── research/             # 第三方开源调研：pi/ 记 pi 自身机制；<feature>/ 按产品分子文件
