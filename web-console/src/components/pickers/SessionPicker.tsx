...
export function SessionPicker({ open, sessions, onSelect, onClose }: SessionPickerProps) {
  return (
    <Modal open={open} onClose={onClose} bodyClass="max-h-[70vh] w-full max-w-lg overflow-auto rounded-lg border border-slate-700 bg-slate-900 p-2">
      <div className="px-2 py-1 text-xs text-slate-500">历史会话（点击打开）</div>
      {sessions.length === 0 && <div className="px-2 py-2 text-sm text-slate-600">（无）</div>}
      {sessions.map((s) => (
        <button key={s.path} onClick={() => onSelect(s.path)} className="block w-full rounded px-2 py-2 text-left hover:bg-slate-800">
